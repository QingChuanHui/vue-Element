...
import { getClientWeekReport } from '@/api/components/admin/index'
...
export default {
  ...
  methods: {
    ...
    load() {
      getClientWeekReport().then(response => {
    ...
    setData(data, type) {
      ...
      } else if (type === 'needDealBillCountList') {
        text = '近七日待处理订单'
        countList = data.map((item) => {
          return item.needDealBillCount
        })
      } else if (type === 'baseClientCompanyCountList') {
        text = '近七日新增门店'
        countList = data.map((item) => {
          return item.baseClientCompanyCount
        })
      }
