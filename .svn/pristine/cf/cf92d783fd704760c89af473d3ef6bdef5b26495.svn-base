<template>
  <div style="height:100%;" >
    ...
    <div style="height:50px;" class="appCenterTop" >
      <el-input  style="width: 120px;" class="filter-item" placeholder="企业名称" v-model="listQuery.clientName" size="mini" >
      </el-input>
    ...
    <el-table :data="page.records" v-loading.body="listLoading" element-loading-text="玩命加载中……"   border fit highlight-current-row  stripe size="mini">
      ...
      <el-table-column align="center" label="操作" width="380" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button type="info"  size="mini" @click="handleCompany(scope.row.id)">门店</el-button>
          <el-button type="warning" size="mini" @click="handleOperator(scope.row.id)">操作员</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
<script>
  import { getClientList, deleteClient } from '@/api/base/client/index'
  export default {
    data() {
      return {
        page: {},
    ...
    methods: {
      ...
      addT() {
        this.$router.push({ path: 'ClientAdd', query: { flag: 1 }})
      },
      handleUpdate(val) {
        this.$router.push({ path: 'ClientUpdate', query: { 'id': val, flag: 2 }})
      },
      handleWatch(val) {
        this.$router.push({ path: 'ClientWatch', query: { 'id': val, flag: 0 }})
      },
      ...
      handleOperator(val) {
        this.$router.push({ path: 'Operator', query: { 'id': val }})
      },
      handleCompany(val) {
        this.$router.push({ path: 'Company', query: { 'id': val }})
      }
    }
  }
</script>
