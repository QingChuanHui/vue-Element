...
<script>
import { getCategoryList, delCategory } from '@/api/manage/homeKind/index'
export default {
  data() {
    return {
      page: {},
  ...
  methods: {
    fetchData() {
      ...
      getCategoryList({ 'pageNo': this.listQuery.pageNo, 'pageSize': this.listQuery.pageSize, 'title': this.listQuery.name }).then(response => {
        this.page = response.page
        this.listLoading = false
      })
    },
    addT() {
      this.$router.push({ path: 'KindAdd' })
    },
    handleUpdate(val) {
      this.$router.push({ path: 'KindEdit', query: { 'id': val }})
    },
    handleWatch(val) {
      this.$router.push({ path: 'KindWatch', query: { 'id': val, flag: 0 }})
    },
