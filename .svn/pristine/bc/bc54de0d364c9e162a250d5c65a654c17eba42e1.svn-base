<template>
  <div style="height:100%;" >
    <div style="height:50px;" class="appCenterTop" > 
      <el-input  style="width: 250px;" class="filter-item" placeholder="项目名称" v-model="listQuery.itemName" size="mini" clearable >
      </el-input>
    ...
    <el-table :data="page.records" v-loading.body="listLoading" element-loading-text="玩命加载中……"  border fit highlight-current-row  stripe size="mini">
      ...
      <el-table-column label="门店价"  align="center"  >
        <template slot-scope="scope">
          {{scope.row.itemOriginalPrice}}
        </template>
      </el-table-column>
...
import { getCompanyItemList, delCompanyItem } from '@/api/company/item/index'
export default {
  data() {
    return {
      page: {},
  ...
  methods: {
    fetchData() {
      ...
      getCompanyItemList({ 'pageNo': this.listQuery.pageNo, 'pageSize': this.listQuery.pageSize, 'itemName': this.listQuery.itemName, 'companyId': this.companyId }).then(response => {
        this.page = response.page
        this.listLoading = false
      })
    },
    addT() {
      this.$router.push({ path: 'ItemAdd', query: { flag: 1 }})
    },
    handleUpdate(val) {
      this.$router.push({ path: 'ItemEdit', query: { 'id': val, flag: 2 }})
    },
    handleWatch(val) {
      this.$router.push({ path: 'ItemWatch', query: { 'id': val, flag: 0 }})
    },
