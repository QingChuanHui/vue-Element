...
<script>
  import { getCategory, setCategory, updCategory } from '@/api/manage/homeKind/index'
  ...
  export default {
    ...
    methods: {
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (this.id) {
              updCategory(this.model).then(response => {
                this.$message({
                  message: '恭喜你，修改成功',
                  type: 'success'
                })
                this.$router.push({ path: 'KindList' })
              })
            } else {
              setCategory(this.model).then(response => {
                this.$message({
                  message: '恭喜你，添加成功',
                  type: 'success'
                })
      ...
      load() {
        getCategory({ 'id': this.id }).then(response => {
          this.model = response.category
